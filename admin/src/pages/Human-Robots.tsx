...
const HumanRobots = () => {
  ...
  const { mutate } = useMutation({
    mutationKey: ["humanrobot"],
    mutationFn: (values: any) => {
      return _axios.post(`/api/humanrobot/create`, {
  ...
  const { data } = useQuery({
    queryKey: ["humanrobot"],
    queryFn: async () => {
      return _axios.get(`/api/humanrobot/content`);
    },
  });
    ...
    <>
      <div className='px-20 py-10'>
        <Card x-chunk='dashboard-04-chunk-1'>
          <CardHeader>
            <CardTitle>Human - Robots - Interaction Section</CardTitle>
